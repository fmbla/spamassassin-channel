...
sub set_config {
  ...
  push(@cmds, {
    setting => 'levenshtein_strict_length',
    default => 0,
    type => $Mail::SpamAssassin::Conf::CONF_TYPE_BOOL,
    }
  );
...
sub check_levenshtein
{
  ...
  if ($self->_check_levenshtein_addr_arr($pms, $from, $tdist, $use_tld, undef, 0, $pms->all_to_addrs())) {
    return 1;
  }
...
{
  my ($self, $pms, $compare, $tdist, $exact_match, $strict_len) = @_;

  my @target = split /\s/, $pms->get("From:name");

  $exact_match = defined $exact_match ? $exact_match : 1;

  if ($self->_check_levenshtein_addr_arr($pms, $compare, $tdist, 0, $strict_len, $exact_match, @target)) {
    return 1;
  }
...
{
  my ($self, $pms, $tdist, $strict_len, $use_tld) = @_;
  ...
  $strict_len = defined $strict_len ? $strict_len : 1;

  if ($self->_check_levenshtein_addr_arr($pms, $compare, $tdist, $use_tld, $strict_len, 0, @froms)) {
    return 1;
  }
...
{
  my ($self, $pms, $compare, $tdist, $strict_len, $use_tld) = @_;

  if ($self->_check_levenshtein_addr_arr($pms, $compare, $tdist, $use_tld, $strict_len, 0, $pms->all_from_addrs_domains())) {
    return 1;
  }
...
{
  my ($self, $pms, $from, $tdist, $use_tld, $strict_len, $exact_match, @to_array) = @_;
  $from = $self->uri_to_domain($from);
  ...
  $strict_len = defined $strict_len ? $strict_len : $pms->{main}->{conf}->{levenshtein_strict_length};
